...
\* Sets and Maps could have been represented in IFEX using the other fundamental types, possibly with a "Layer" that adds on the special behavior of Sets and Maps.  (How the types can be simulatedi in environments that don't support them natively is described in the next chapter).  Despite this, it was decided that the behavior of set and map types are common enough that IFEX Core IDL should support them as fundamental types.
...
| IFEX Fundamental type | If Other supports the type: | If Other does *not* support the type |
| --------------------- | ------------------------------------ | ------------------------------------------------------------------|
